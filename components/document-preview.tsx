...
export default function DocumentPreview({ formData }: DocumentPreviewProps) {
  ...
  const getEventDescription = () => {
    if (formData.type_prestation === "evenement_sportif") {
      return `${formData.evenement || "Roland-Garros"} - ${formData.court} - ${formData.categorie}`
    }
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Aperçu de votre attestation</h2>
      ...
      <Card className="bg-white shadow-lg border-2 border-gray-200">
        <CardHeader className="text-center bg-gray-50 border-b">
          <CardTitle className="text-xl font-bold text-gray-900">ATTESTATION DE PRESTATION DE SERVICE</CardTitle>
          <p className="text-sm text-gray-600">Établie le {new Date().toLocaleDateString("fr-FR")}</p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="bg-orange-50 p-4 rounded-lg space-y-2">
              <p>
                <strong>Montant :</strong> <span className="text-xl font-bold text-orange-600">{formData.prix} €</span>
              </p>
              <p>
                <strong>Mode de paiement :</strong>{" "}
                <Badge variant="outline">{formData.mode_paiement === "especes" ? "Espèces" : "Virement"}</Badge>
              </p>
