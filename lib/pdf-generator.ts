...
export function generateAttestationPDF(data: AttestationData): jsPDF {
  ...
  doc.text("ATTESTATION DE PRESTATION DE SERVICE", pageWidth / 2, y, { align: "center" })
  ...
  y += 6
  doc.text(`Date : ${new Date(data.date).toLocaleDateString("fr-FR")}`, margin, y)
  ...
  const faitAtext = `Fait à : ${data.ville}    Le : ${new Date(data.date).toLocaleDateString("fr-FR")}`
  ...
  doc.setFont("helvetica", "normal")
  doc.text(
    "TGZ S.A.S – contact@tgz.events – 4 rue de sontay, 75016 Paris, France",
    pageWidth / 2,
    footerY,
    { align: "center" }
  )
